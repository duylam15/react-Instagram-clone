...
const Login = () => {
    ...
    const [errors, setErrors] = useState<{ username?: string; password?: string }>({}); // Đổi key từ email thành username
    ...
    const validateInputs = () => {
        ...
        if (!username) {
            newErrors.username = "Vui lòng nhập tên đăng nhập!"; // Đổi thông báo lỗi
            isValid = false;
        }
    ...
    const handleUsernameChange = (e: React.ChangeEvent<HTMLInputElement>) => { // Đổi từ handleEmailChange thành handleUsernameChange
        setUsername(e.target.value);
        if (errors.username) {
            setErrors((prev) => ({ ...prev, username: "" })); // Đổi key từ email thành username
        }
    };
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            const res = await callLogin(username, password); // Đổi từ email thành username
            if (res?.data) {
                ...
                navigate("/home"); // Sửa từ /register thành /home
            } else {
            ...
            <div className="flex flex-col items-center w-[350px] bg-white p-8 rounded-lg shadow-md border border-gray-300">
                ...
                <form onSubmit={handleLogin} className="flex flex-col w-full">
                    <div className="mb-2">
                        <input
                            type="text"
                            placeholder="Tên đăng nhập" // Đổi placeholder từ Email thành Tên đăng nhập
                            value={username} // Đổi từ email thành username
                            onChange={handleUsernameChange} // Đổi từ handleEmailChange thành handleUsernameChange
                            className={`p-2 border rounded-md text-sm w-full ${
                                errors.username ? "border-red-500" : "border-gray-300" // Đổi key từ email thành username
                            } focus:outline-none focus:border-black`}
                        />
                        {errors.username && <p className="text-red-500 text-xs mt-1">{errors.username}</p>} {/* Đổi key từ email thành username */}
                    </div>
